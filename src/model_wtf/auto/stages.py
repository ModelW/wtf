...
GEN_HEADER = "# GENERATED by model-wtf (agent) -- do not edit\n"
...
def discover_items(ctx: UnitContext) -> list[WorkItem]:
    """One item per unit whose surface changed since the last discovery."""
    unit_gen = ctx.folder / "elements" / "unit.gen.yaml"
    current = tree_sha(ctx.root, ctx.context_dir, exclude=ctx.folder)
    if unit_gen.is_file():
        previous = (yaml.safe_load(unit_gen.read_text(encoding="utf-8")) or {}).get(
            "source_sha"
        )
        if previous == current:
            return []
    ...
    def write(answer: AgentOutput, _model: str | None) -> list[Path]:
        ...
        return _write_discovery(ctx, answer, current)
...
def _write_discovery(
    ctx: UnitContext, answer: DiscoverOutput, source_sha: str
) -> list[Path]:
    written: list[Path] = []
    elements = ctx.folder / "elements"
    ...
    written.append(
        _write_gen(
            elements / "unit.gen.yaml",
            {
                ...
                "stack": answer.stack,
                "routes": [r.model_dump() for r in answer.routes],
                "tasks": [t.model_dump() for t in answer.tasks],
            },
        )
    )
    data_dir = ctx.folder / "data"
    data_dir.mkdir(exist_ok=True)
    for model in answer.models:
        object_id = model.id.lower()
        written.append(
            _write_gen(
                data_dir / f"{object_id}.gen.yaml",
                {
                    "by": "agent",
                    "sources": [f"store:{model.id}"],
                    "fields": {
                        f.name: {
                            "type": f.type,
                            **({"opaque": True} if f.opaque else {}),
                            **(
                                {"candidate_contents": f.candidate_contents}
    ...
    recipients_dir = ctx.folder / "recipients"
    recipients_dir.mkdir(exist_ok=True)
    for egress in answer.egress:
        written.append(
            _write_gen(
                recipients_dir / f"{egress.slug}.gen.yaml",
                {
                    "by": "agent",
                    "detection": {
                        "packages": egress.sdks,
                        "hosts": egress.hosts,
                        "env": egress.env_keys,
                    },
                    "provenance": egress.provenance,
                },
            )
        )
    return written
...
def _write_gen(path: Path, data: dict[str, Any]) -> Path:
    ...
    path.write_text(
        GEN_HEADER + yaml.safe_dump(data, sort_keys=False, allow_unicode=True),
        encoding="utf-8",
    )
    return path
