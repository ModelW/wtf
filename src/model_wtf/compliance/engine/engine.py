...
def apply_to_folder(
    ...
    *,
    ...
    """Write applicability, then reconcile ledgers and findings.

    Per element: the ``.gen.yaml`` gets the applicability lists, the
    checkpoint set is synced (new -> ``unknown``, dropped -> removed,
...
def _write_gen(path: Path, element: Element) -> None:
    ...
    text = GEN_HEADER + yaml.safe_dump(data, sort_keys=False, allow_unicode=True)
